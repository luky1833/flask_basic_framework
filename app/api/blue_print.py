...
from app.controller.base import SwxJsonHandler
import json
# 时间戳处理
from app.utils.CJsonEncoder import CJsonEncoder
class Hello_v1(SwxJsonHandler):
    def __init__(self):
        # 设置参数
        super().__init__()
    ...
    def get(self):
        try:
            ...
            # print()
            result = self.write_json(200, '获取成功', data=data)
            return result, 200
        except Exception as e:
            log.e('blue_print.Hello_v1.get:' + str(e) + '\n')
            error = dict()
            error['error_503'] = str(e)
            result = self.write_json(500, '获取参数失败')
            return result, 500
